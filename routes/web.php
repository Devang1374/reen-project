...
Route::post('/midchack',function(Request $request){
    $validated = $request->validate([
        'name' => ['required', 'max:10'],
        'email' => ['required']
    ]);
    return "success";
})->middleware(myMiddleware::class)->name('midchack');

Route::view('/midchack', 'midchack');

Route::get('/download',function (){
    return response()->download('http://127.0.0.1:8000/storage/files/tamp');
});